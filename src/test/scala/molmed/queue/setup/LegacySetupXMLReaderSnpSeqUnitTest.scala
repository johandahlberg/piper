...
class LegacySetupXMLReaderSnpSeqUnitTest {
	...
	def TestGetSamples() = {
	    ...
	    expected("1") = Seq(new LegacySample("1", setupXMLReader, illuminaXMLReportReader,1, runFolderName)) 
	...
	def TestGetSameSampleFromSeveralRunFolders() = {	   
	    ...
	    val illuminaXMLReportReader: IlluminaXMLReportReaderAPI = new IlluminaXMLReportReaderStub()
	    val expected:  scala.collection.mutable.Map[String, Seq[SampleAPI]] = scala.collection.mutable.Map.empty[String, Seq[SampleAPI]]
	    expected("1") = Seq(new LegacySample("1", setupXMLReader, illuminaXMLReportReader, 1, runFolderName1))
	    expected("1") :+= new LegacySample("1", setupXMLReader, illuminaXMLReportReader, 1, runFolderName2)
	...
	def TestGetSameSampleFromSeveralLanesInSameRunFolder() = {	   
	    ...
	    val expected:  scala.collection.mutable.Map[String, Seq[SampleAPI]] = scala.collection.mutable.Map.empty[String, Seq[SampleAPI]]
	    expected("1") = Seq(new LegacySample("1", setupXMLReader, illuminaXMLReportReader, 1, runFolderName1))
	    expected("1") :+= new LegacySample("1", setupXMLReader, illuminaXMLReportReader, 1, runFolderName1)
