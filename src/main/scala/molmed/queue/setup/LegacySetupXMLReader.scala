            ...
            for(sampleNode <- sampleNodes) {
	            ...
	            for (lane <- illuminaXMLReportReader.getLanes(sampleName)) {	 	                
		            ...
		            if(!samples.contains(sampleName))
		                samples(sampleName) = Seq(new LegacySample(sampleName, this, illuminaXMLReportReader, lane, runFolderName))		                		            
		            else
		                samples(sampleName) :+= new LegacySample(sampleName, this, illuminaXMLReportReader, lane, runFolderName)	            		                
		        }          	            
	        }
        }
    	samples.toMap
